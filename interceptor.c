...
void my_exit_group(int status)
{
	...
	del_pid(current->pid); // remove the current pid from all lists
	spin_unlock(&my_table_lock);

	return orig_exit_group(status); // call the original exit_group function
...
asmlinkage long interceptor(struct pt_regs reg) {
	...
	spin_lock(&my_table_lock);
	// to check if the syscall is being monitored for current pid.
	// two cases for being monitored: all pids are monitored in this syscall or some pids are monitored and the current pid is in the pid list
	...
	spin_unlock(&my_table_lock);
	return table[reg.ax].f(reg); // call the original function call
}

/*
* Helper function for the case that cmd is REQUEST_SYSCALL_INTERCEPT
* Include intercept steps and error checking 
*/
int start_intercept(int syscall, int pid) {
	if (current_uid() != 0) {  // Check if calling process is root
		return -EPERM;
	}

	spin_lock(&my_table_lock);
	if (table[syscall].intercepted == 1) { // check if the syscall has been intercepted already
		spin_unlock(&my_table_lock);
		return -EBUSY;
	}
	spin_unlock(&my_table_lock);
	...
	// set the system table writable
	set_addr_rw((unsigned long)sys_call_table);
	// intercept
	sys_call_table[syscall] = interceptor;
	table[syscall].intercepted = 1;
	// set the system table read only
	set_addr_ro((unsigned long)sys_call_table);
	spin_unlock(&sys_call_table_lock);
	return 0;
}
/*
* Helper function for the case that cmd is REQUEST_SYSCALL_REALEASE
* Include de-intercept steps and error checking 
*/
int release_intercept(int syscall, int pid) {
	...
	spin_lock(&my_table_lock);
	if (table[syscall].intercepted == 0) { // check the syscall has been intercepted: cannot release if it's not intercepted
		spin_unlock(&my_table_lock);
		return -EINVAL;
	}
...
/*
* Helper function for the case that cmd is REQUEST_START_MONITORING
* Include steps for both pid = 0 and pid !=0,  and error checking 
*/
int start_monitor(int syscall, int pid) {
	if ( current_uid() != 0) { // Check if calling process is root
		// the calling process is not root
		if ( pid == 0 || check_pids_same_owner(current->pid, pid) != 0) { // check if pid = 0 and if the owners of current pid and request pid are the same
			return -EPERM;
		}
	}

	if (pid < 0 || (pid != 0 && pid_task(find_vpid(pid), PIDTYPE_PID) == NULL)) { // check if the pid is valid
		return -EINVAL;
	}

	spin_lock(&my_table_lock);
	if (table[syscall].intercepted == 0) { // check if the sys call is intercepted
		spin_unlock(&my_table_lock);
		return -EINVAL;
	}

	if (pid !=  0) {// check if the specific pid has been monitored
		// two cases: some pids are monitored and pid is in the pid list; 
		// all pids are monitored except the black list --> it's not in black list
	...
	if (pid == 0) { // the case that start monitoring all pids
		if (table[syscall].listcount == 0 && table[syscall].monitored == 2) { // Blacklist not empty
											 // => already monitoring all pids
			spin_unlock(&my_table_lock);
			return -EBUSY;
		}

		destroy_list(syscall); // clear up pid list
		table[syscall].monitored = 2; 
		// BOUNS CASE
		// my_list becomes blacklist after destorying the original list.
		spin_unlock(&my_table_lock);
		return 0;

	} else { // start monitoring the pid

		if (table[syscall].monitored == 2) { // this pid is in the black list
			// BONUS CASE
			// Delete pid from blacklist => start monitoring pid
			if (del_pid_sysc(pid, syscall) == -EINVAL) { // check if the pid is successfully deleted from the pid list which is the black list in this case
				spin_unlock(&my_table_lock);
				return -EINVAL;
			}
			spin_unlock(&my_table_lock);
		...
		// monitored == 1
		if (add_pid_sysc(pid, syscall) == -ENOMEM) { // check if the pid is successfully added to the pid list
			spin_unlock(&my_table_lock);
			return -ENOMEM;
		}

		if (table[syscall].monitored == 0) { // the case that the pid is the first pid to be monitored
...
/*
* Helper function for the case that cmd is REQUEST_STOP_MONITORING
* Include steps for both pid = 0 and pid !=0,  and error checking 
*/
int stop_monitor(int syscall, int pid) {
	...
	// there is not pid has been monitored, cannot stop monitoring
   if (table[syscall].monitored == 0) {
	   spin_unlock(&my_table_lock);
	   return -EINVAL;
   }

   if (pid != 0) { // the case fir a specific pid
   		// check if the pid is monitored
   		// two cases: some pids are monitored and pid is not in the pid list; 
		// all pids are monitored except the black list --> it's in black list
   ...
   if (pid == 0) { // the case that stop monitoring all pids
   ...
   } else { // pid != 0   Requesting specific pid
	   if ( table[syscall].monitored == 2 ) { // BOUNS CASE: stop monitoring a pid from a syscall that has monitored all pid
	   ...
	   // monitored == 1
	   if (del_pid_sysc(pid, syscall) == -EINVAL) { // check if the pid is successfully deleted from the pid list
		   spin_unlock(&my_table_lock);
		   return -EINVAL;
	   }
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
	...
	// error checking for valid syscall
	...
	switch(cmd) {
		case REQUEST_SYSCALL_INTERCEPT:
			return start_intercept(syscall, pid);

		case REQUEST_SYSCALL_RELEASE:
			return release_intercept(syscall, pid);
...
static int init_function(void) {
	...
	for (i = 0 ; i < NR_syscalls; i++) { // initialize the table
		spin_lock(&my_table_lock);
...
static void exit_function(void)
{
	...
	spin_lock(&sys_call_table_lock);
	set_addr_rw((unsigned long)sys_call_table); // set to writable

	for (i = 0 ; i < NR_syscalls; i++) {// deintercept  all syscalls and destroy all pid lists
		spin_lock(&my_table_lock);
	...
	// restore MY_CUSTOM_SYSCALL and __NR_exit_group to the original syscall
	sys_call_table[MY_CUSTOM_SYSCALL] = orig_custom_syscall;
	sys_call_table[__NR_exit_group] = orig_exit_group;
	set_addr_ro((unsigned long)sys_call_table); // set it to read only
